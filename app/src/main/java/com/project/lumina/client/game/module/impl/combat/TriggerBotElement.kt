...
class TriggerBotElement(
    iconResId: Int = AssetManager.getAsset("ic_bullseye_arrow_black_24dp")
) : Element(
    name = "TriggerBot",
    ...
    // FOV 옵션
    private val useFov by boolValue("Use FOV", true)
    private val fov by floatValue("FOV", 30f, 5f..180f)
    ...
    private fun getTargets(): List<Entity> =
        session.level.entityMap.values.filter {
    ...
    private fun isInFov(target: Entity): Boolean {
        ...
        val delta = wrapAngle(player.vec3Rotation.y - targetYaw)

        return abs(delta) <= fov
    }
    ...
    private fun Entity.isValid(): Boolean = when (this) {
        ...
        is Player -> playerOnly && !isBot()
        is EntityUnknown -> mobsOnly && !isShadow()
        else -> false
    }

    private fun Player.isBot(): Boolean {
        val name = session.level.playerMap[uuid]?.name
        return name?.toString()?.isBlank() ?: true
    }

    private fun EntityUnknown.isShadow(): Boolean =
        identifier == "hivecommon:shadow"

    private fun Vector3f.norm(): Vector3f {
        val len = length()
        return if (len != 0f)
            Vector3f.from(x / len, y / len, z / len)
        else this
    }
}
